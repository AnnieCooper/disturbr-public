...
create_attribution_model <- function(data, p_train = 0.6, p_test = 0.4, 
  ...
  ### NEED TO SAMPLE ALL DISTURBANCES PRESENT
  set.seed(3522)
  ...
  data_train <- data %>%
    ...
    sample_frac(0.6) %>%
    as.data.table
  data_test <- data[!(data$index %in% data_train$index),]
  ...
  print(paste('Percentage disturbed total:', 
              nrow(data[as.numeric(as.character(data$disturbance)) >= '2',]) / nrow(data)))
  print(paste('Percentage disturbed in training:', 
              nrow(data_train[as.numeric(as.character(data_train$disturbance)) >= '2',]) / nrow(data_train)))
  print(paste('Percentage disturbed in testing:', 
              nrow(data_test[as.numeric(as.character(data_test$disturbance)) >= '2',]) / nrow(data_test)))
  ...
  data_train <- data_train[, grep('^i.', names(data_train)) := NULL, ]
  data_test <- data_test[, grep('^i.', names(data_test)) := NULL, ]
  
  # remove date and location columns from training dataset
  data_train <- data_train[, c('longitude', 'latitude', 'date_min_slope', 'date_min', 'dist_date',
                               'd_dist_date_med', 'd_date_min_med', 'splits') := NULL, ]
...
### TOMORROW - NEED TO FINISH ATTRIBUTION MODEL CODE, DECIDE HOW TO DEAL WITH MISSING
### CLASSES IN MODELS, FINISH TESTING WITH JOB, RUN FOR EACH SCENE THAT HAS DATA,